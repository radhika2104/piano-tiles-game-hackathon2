  ...
  currentRowOnCurrentBoard.addEventListener("click", function (event) {
    ...
    if (element.style.backgroundColor == "black") {
      const audio = document.querySelector(
        `audio[data-key="${element.textContent}"]`
      );
...
function appendAndDeleteRow(gamePlaying) {
  ...
  if (timeElapsed > gameScore) {
    console.log(
      "game stopped due to no click in appendAndDeleteRow as time elapsed!!"
    );
    gamePlaying = false;
    clearInterval(myInterval);
...
function clickonStart() {
  ...
  currentRow.addEventListener("click", function (event) {
    ...
    const audio = document.querySelector(
      `audio[data-key="${cell.textContent}"]`
    );
...
function endGamePopMessage() {
  ...
  endGamePop.innerHTML +=
    `<p class="game-over-message">Game Over!</p><p class="game-over-score">Game Score:` +
    gameScore +
    `</p><button class="play-btn">Play Again</button>`;
